...
## 李晨韬
- 新增了录像模块，可以用于录制视频，并在录制结束后指定位置播放，用脚本编写，可为后续复杂过程的子过程
- 新增了人脸识别签到功能，摄像头每检测到一个人脸，就进行一次欢迎播报
- 计划对人脸识别做更深入的探索

## 何东阳
- 美化了窗口的自定义关闭图标，使得qt界面看起来更加自然
- 初步熟悉机械臂操作手册，为之后的控制做准备
...
- 研究机械臂源码，下载keil编译环境
- 看完所有视频介绍
- 编写arduino程序试图实现与STM32板子的通讯，但仅仅能看到发出去的信息不能获得反馈，需要进一步和学长交流
- 研究逆运动学，以达到机械臂抓取指定位置物体的效果

## 李晨韬
- 对人脸签到模块进行了更深入的开发，加入照相模块用于图片的回传，即在检测到人脸后进行语音播报的同时，将人脸数据传输回主机供主机使用者查验
- 对语音控制运动模块进行了更深入的开发，以前语音识别的间隔为10s，这样就会造成机器人至少需要10s才能转变一次反向，如果语音识别检测错误，那所需花费的时间将会更多。为此，我深入讯飞语音识别的底层接口，找到在IAT_node.cpp里，控制了语音时长输入的参数，将其修改为5后，每次识别只需要5s，可以改的更快，但尽量最好不要小于3s，否则会造成识别时长过短而识别不到的后果。
- 对语音输出模块进行了更深入的开放，以前发现说一段由strSpeak合成的话，无论多长，都只能读10s，这严重限制了语音播报的功能。我首先进入讯飞的语音输出模块tts_node里寻找问题，发现tts_node主要负责语音的合成规则，其中并没有对语音时长进行限制。经过更深层次的底层搜索，我发现是include的soundplay部分有问题，最终找到了soundplay_node.py，将其中一个原值为10的参数修改为20，发现此时就能正常读取20s的语音合成内容，非常好用
- 希望ROS机器人对于相关机器人接口能提供更进一步的手册
